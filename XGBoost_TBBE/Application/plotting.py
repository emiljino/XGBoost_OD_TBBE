...
def raceEventPlot(filename):
    ...
    for i in range(1, NUM_OF_COMPETITORS + 1):
        ys.append(i)
    dataframe.plot(
        x="Time", xlabel="Time (s)", y=ys, ylabel="Distance (m)", kind="line"
    )
...
def price_histories(filename):
    ...
    dataframe.plot(
        x="Time", y=ys, xlabel="Time (s)", ylabel="Odds", logy=True, drawstyle="steps"
    )
...
def price_spreads(filename):
    ...
    dataframe.plot(
        x="Time", y=ys, xlabel="Time (s)", ylabel="Log Price", drawstyle="steps"
    )
...
def histogram(path, time_interval=0.5):
    ...
    ax = sns.barplot(
        np.round(np.arange(0, len(bars) * time_interval, time_interval), 1), bars
    )
