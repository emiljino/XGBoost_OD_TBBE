...
class BettingAgent:
    def __init__(
        ...
        name,
        lengthOfRace,
        endOfInPlayBettingPeriod,
        influenced_by_opinions,
        local_opinion,
        uncertainty,
        lower_op_bound,
        upper_op_bound,
        ...
        self.omega = random.uniform(
            0, 1
        )  # 'influenced_by_opinions' bettors bias weight they put on opinion vs their coded strategy when
        # betting.
    ...
    def bookkeep(self, trade, type, order, time):
        ...
        if type == "Layer":
            self.amountFromTransactions += (trade["odds"] * trade["stake"]) + trade[
                "stake"
            ]
...
class Agent_Opinionated_Leader_Wins(BettingAgent):
    ...
    def respond(self, time, markets, trade):
        ...
        if (
            self.bettingTime <= self.raceTimestep
            and self.raceTimestep % self.bettingInterval == 0
        ):

            sortedComps = sorted(
                (self.currentRaceState.items()), key=operator.itemgetter(1)
            )
            ...
            if markets[self.exchange][compInTheLead]["backs"]["n"] > 0:
                quoteodds = max(
                    MIN_ODDS,
                    markets[self.exchange][compInTheLead]["backs"]["best"] - 0.1,
                )
            else:
            ...
            order = Order(
                ...
                quoteodds,
                random.randint(self.stakeLower, self.stakeHigher),
                markets[self.exchange][compInTheLead]["QID"],
                time,
            )
...
class Agent_Opinionated_Underdog(BettingAgent):
    ...
    def observeRaceState(self, timestep, compDistances):
        ...
        if self.bettingTime <= self.raceTimestep:
            ...
            if (
                float(compInTheLead[1])
                <= (float(compInSecond[1]) + float(self.threshold))
                and compInTheLead[0] != self.compInTheLead
            ):
    ...
    def respond(self, time, markets, trade):
        ...
        if (
            self.numOfBets >= 10
            or self.raceStarted == False
            or self.bettingPeriod == False
        ):
            return order
        if self.bettingTime <= self.raceTimestep:
            if self.job == "back_underdog":
                ...
                if markets[self.exchange][self.compInSecond]["backs"]["n"] > 0:
                    quoteodds = max(
                        MIN_ODDS,
                        markets[self.exchange][self.compInSecond]["backs"]["best"]
                        - 0.1,
                    )
                else:
                    quoteodds = markets[self.exchange][self.compInTheLead]["backs"][
                        "worst"
                    ]
                ...
                order = Order(
                    ...
                    quoteodds,
                    random.randint(self.stakeLower, self.stakeHigher),
                    markets[self.exchange][self.compInSecond]["QID"],
                    time,
                )
                self.job = "lay_leader"
                ...
                if markets[self.exchange][self.compInTheLead]["lays"]["n"] > 0:
                    ...
                    quoteodds = (
                        markets[self.exchange][self.compInTheLead]["lays"]["best"] + 0.1
                    )
                else:
                    quoteodds = markets[self.exchange][self.compInTheLead]["lays"][
                        "worst"
                    ]
                ...
                order = Order(
                    ...
                    quoteodds,
                    random.randint(self.stakeLower, self.stakeHigher),
                    markets[self.exchange][self.compInTheLead]["QID"],
                    time,
                )
                self.job = None
    ...
    def respond(self, time, markets, trade):
        ...
        if marketsFave == self.marketsFave:
            ...
            order = Order(
                ...
                quoteodds,
                random.randint(self.stakeLower, self.stakeHigher),
                markets[self.exchange][marketsFave]["QID"],
                time,
            )
...
class Agent_Opinionated_Linex(BettingAgent):
    ...
    def predict(self):
        ...
        for i in range(NUM_OF_COMPETITORS):
            if i in self.injuredCompetitors:
                continue
            ...
            if (
                self.predictedWinner == None
                or self.predictedResults[i] < predictedWinnerTime
            ):
                self.predictedWinner = i
                predictedWinnerTime = self.predictedResults[i]
            if (
                self.predictedLoser == None
                or self.predictedResults[i] > predictedLoserTime
            ):
    ...
    def observeRaceState(self, timestep, compDistances):
        ...
        if (
            len(self.raceHistoryDists[0]) > (self.n + self.recordingTime)
            and self.predicted == False
        ):
            self.predict()
    ...
    def respond(self, time, markets, trade):
        ...
        order = None
        if self.predicted == False or self.bettingPeriod == False:
            return order
        ...
        if self.job == "back_pred_winner":
            ...
            if markets[self.exchange][self.predictedWinner]["backs"]["n"] > 0:
                ...
                quoteodds = max(
                    MIN_ODDS,
                    markets[self.exchange][self.predictedWinner]["backs"]["best"] - 0.1,
                )
            else:

                quoteodds = markets[self.exchange][self.predictedWinner]["backs"][
                    "worst"
                ]
            ...
            if markets[self.exchange][self.predictedLoser]["lays"]["n"] > 0:
                quoteodds = (
                    markets[self.exchange][self.predictedLoser]["lays"]["best"] + 0.1
                )
            else:
    ...
    def getInPlayOrder(self, time, markets):
        ...
        if (self.raceTimestep % self.updateInterval) == 0:
            ...
            if self.latest_odds is None:
                ...
                local_op = (
                    (1 - self.strategy_weight) * self.local_opinion
                    + self.strategy_opinion * self.strategy_weight
                )
                self.local_opinion = local_op
                self.opinion = (
                    self.a1 * self.local_opinion
                    + self.a2 * self.global_opinion
                    + self.a3 * self.event_opinion
                )
            ...
            for i in range(len(odds)):
                    ...
                    if self.event_opinion == 1 or self.event_opinion == 0:
                        ...
                        if len(self.currentRaceState.values()) > 0:
                            ...
                            for c in self.currentRaceState.values():
                                total = (
                                    total
                                    + (self.lengthOfRace / self.lengthOfRace - c) ** 2
                                )
                            c_event_opinion = (
                                self.lengthOfRace
                                / (self.lengthOfRace - self.currentRaceState[i])
                            ) ** 2 / total
                        else:
                            c_event_opinion = 1 / NUM_OF_COMPETITORS

                c_opinion = (
                    self.a1 * c_local_opinion
                    + self.a2 * c_global_opinion
                    + self.a3 * c_event_opinion
                )
                ...
                if quoteodds == MAX_ODDS:
                    ...
                    if markets[self.exchange][i]["backs"]["best"] is not None:
                        quoteodds = (
                            markets[self.exchange][i]["backs"]["best"] + self.layDelta
                        )
                        lay_odds = (
                            self.omega * lay_opinionated_odds
                            + (1 - self.omega) * quoteodds
                        )
                        order = Order(
                            self.exchange,
                            self.id,
                    ...
                    if quoteodds < markets[self.exchange][i]["backs"]["best"]:
                        ...
                        back_odds = (
                            self.omega * back_opinionated_odds
                            + (1 - self.omega) * quoteodds
                        )
                        order = Order(
                            self.exchange,
...
class XGBoost1BettingAgent(BettingAgent):
    ...
    def make_decision(self, time, stake, distance, rank):
        ...
        # print(df)
        prediction = self.xgb_loaded_model.predict(dmatrix_data)[
            0
            ...
            for rank, (competitor, distance) in enumerate(sortedComps):
                ...
                if decision == 1:  ## Decision = back
                    if markets[self.exchange][competitor]["backs"]["n"] > 0:
                        quoteodds = max(
                            MIN_ODDS,
                            markets[self.exchange][competitor]["backs"]["best"] - 0.1,
                        )
                    else:
                    ...
                    order = Order(
                        ...
                        quoteodds,
                        random.randint(self.stakeLower, self.stakeHigher),
                        markets[self.exchange][competitor]["QID"],
                        time,
                    )
                    ...
                    if markets[self.exchange][competitor]["lays"]["n"] > 0:
                        quoteodds = (
                            markets[self.exchange][competitor]["lays"]["best"] + 0.1
                        )
                    else:
