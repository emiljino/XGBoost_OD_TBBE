...
class RaceAttributes:
    ...
    def randomise(self):
        ...
        elif self.length > 1445 and self.length <= 2890:
            self.race_type = "medium"
        else:
            self.race_type = "long"
...
class Simulator:
    ...
    def __init__(self, numOfCompetitors, comps=None, raceAttributes=None):
        ...
        self.raceSplit = {
            "start": (0, self.race_attributes.length / 3),
            "middle": (
                self.race_attributes.length / 3,
                (self.race_attributes.length / 3) * 2,
            ),
            "end": ((self.race_attributes.length / 3) * 2, self.race_attributes.length),
        }
        self.runningStyleImpactChanged = []
        self.finalStretchDist = {"short": 550, "medium": 750, "long": 1000}
    ...
    def updateEnergy(self, increases):
        ...
        for i in range(len(sortedComps) - 1):
            ...
            if sortedComps[i + 1].distance <= sortedComps[i].distance + 5:
                sortedComps[i].energy = sortedComps[i].energy - (
                    0.1 * increases[sortedComps[i].id]
                )
            else:
                sortedComps[i].energy = (
                    sortedComps[i].energy - increases[sortedComps[i].id]
                )

        sortedComps[len(sortedComps) - 1].energy = (
            sortedComps[len(sortedComps) - 1].energy
            - increases[sortedComps[len(sortedComps) - 1].id]
        )
    ...
    def updateResponsiveness(self):
        ...
        def runningStyleImpact(self, c):
            ...
            middleRange = (
                int((NUM_OF_COMPETITORS / 3)),
                int((NUM_OF_COMPETITORS / 3) * 2),
            )
            bottomRange = (0, int((NUM_OF_COMPETITORS / 3)))
            for i in range(bottomRange[0], bottomRange[1]):
                bottomThird.append(sortedComps[i])
            for i in range(middleRange[0], middleRange[1]):
                middleThird.append(sortedComps[i])
            for i in range(topRange[0], topRange[1]):
                topThird.append(sortedComps[i])
            if self.raceSplit["start"][0] <= c.distance <= self.raceSplit["start"][1]:
                if c in topThird:
                    if (
                        c.running_style == "frontrunner"
                        and c.id not in self.runningStyleImpactChanged
                    ):
            ...
            if self.raceSplit["middle"][0] <= c.distance <= self.raceSplit["middle"][1]:
                ...
                if (
                    c.running_style == "frontrunner"
                    and c.id in self.runningStyleImpactChanged
                ):
                    c.responsiveness = c.responsiveness / random.gauss(1.2, 0.05)
        ...
        def finalStretch(self, c):
            if c.id in self.finished:
                return
            if (
                c.distance
                >= self.race_attributes.length
                - self.finalStretchDist[self.race_attributes.race_type]
                and c.id not in self.finalStretchIncreases
            ):
                # in final stretch
    ...
    def calcInterference(self, c, increases):
        ...
        for other in self.competitors:
            if other == c or other in self.injuredCompetitors:
                continue
    ...
    def dynamicDistractions(self, c):
        def stumble():
            if random.randint(1, 100) == 50:
                return float(random.gauss(0.6, 0.1))
            else:
                return 1
    ...
    def updateRaceState(self, timestamp):
        ...
        for c in self.competitors:
            ...
            increase = (
                ...
                * c.responsiveness
                * dynamicDistractions
                * (c.alignment * random.randint(c.speed[0], c.speed[1]))
            )
        ...
        for c in self.competitors:
            ...
            if cappedDist == -1:
                c.distance = min(
                    self.race_attributes.length, c.distance + increases[c.id]
                )
            else:
                c.distance = min(
                    self.race_attributes.length,
                    min(cappedDist, c.distance + increases[c.id]),
                )
    ...
    def run(self, fn):
        ...
        print("Write to FN ", fn)
        if fn != None:
            self.writeToFile(fn)

    # Write race state to file
    def writeToFile(self, name):
        print("Write to File Name ", name)
        header = ["Time"]
        for c in self.competitors:
    ...
    def printInitialConditions(self):
        ...
        print(
            ...
            + str(conds.length)
            + " , Undulation: "
            + str(conds.undulation)
            + " , Temperature: "
            + str(conds.temperature)
        )
    ...
    def printCompPool(self):
        ...
        for c in comps:
            ...
            print(
                ...
                + str(prefs.length)
                + " , Undulation: "
                + str(prefs.undulation)
                + " , Temperature: "
                + str(prefs.temperature)
            )
